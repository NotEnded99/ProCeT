...
class VanDerPolOscillator(DynamicalSystem):
    ...
    def compute_lipschitz_constant(self, R):
        ...
        L2 = max(1 + 2 * self.mu * R * R, self.mu * (1 + R**2))  # From the second row
        return max(L1, L2)
    ...
    def max_gradient_norm(self, c, r):
        ...
        # From the second row
        L21 = max(*[abs(-1 - 2 * self.mu * corner[0] * corner[1]) for corner in corners])
...
class Quadcopter(DynamicalSystem):
    ...
    def __init__(
        ...
        if self.orientation:
            ...
            self.delta = np.array([0.1, 0.1, 0.1, 1.0, 1.0, 1.0, 5.0, 5.0, 5.0, 5.0])  # Orientation  # Angular velocity  # Squared angular velocities of rotors
        else:
            # Define state dimension for orientation and angular velocity only (reduced model)
            ...
            self.delta = np.array([1.0, 1.0, 1.0, 5.0, 5.0, 5.0, 5.0])  # Angular velocity  # Squared angular velocities of rotors
    ...
    def compute_dynamics(self, x, translator):
        ...
        if self.orientation:
            derivatives = translator.stack([droll, dpitch, dyaw, domega_x, domega_y, domega_z])
        else:
            derivatives = translator.stack([domega_x, domega_y, domega_z])
...
class VortexShedding4D(DynamicalSystem):
    ...
    def compute_dynamics(self, x, translator):
        ...
        dx4 = -damping * x4 + self.coupling * (translator.sin(x1 * x3) - 0.5 * x2 * translator.pow(x3, 2) + 0.2 * translator.cos(frequency * x2))
...
class QuadraticSystem(DynamicalSystem):
    ...
    def compute_dynamics_discrete(self, x_current, translator, dt):
        ...
        if abs(2 * self.mu - self.lam) < 1e-9:
            ...
            C = x2_0
            x2_next = C * np.exp(self.lam * dt) - self.lam * translator.pow(x1_0, 2) * dt * np.exp(self.lam * dt)
        else:
            # General case: 2μ != λ
            A = self.lam * translator.pow(x1_0, 2) / (2 * self.mu - self.lam)
            # C = (x2_0 + A) / translator.exp(self.lam * 0) which is x2_0 + A
            C = x2_0 + A
            x2_next = C * np.exp(self.lam * dt) - A * np.exp(2 * self.mu * dt)
...
class LowThrustSpacecraft(DynamicalSystem):
    ...
    def __init__(
        ...
        self.delta = np.array(
            [
                1,
                np.pi,  # Position deltas (r, θ)
                5,
                5,  # Velocity deltas (vr, vθ)
                5,  # Mass delta
                0.5,
                np.pi,  # Control input deltas (thrust_magnitude, thrust_angle)
            ]
        )
        self.epsilon = 0.01
    ...
    def compute_dynamics(self, x, translator):
        ...
        derivatives = translator.stack([dr, dtheta, dv_r, dv_theta, dmass])  # Position derivatives  # Velocity derivatives  # Mass derivative
