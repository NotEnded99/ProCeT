...
def train_nn(dynamics_model, learning_rate=0.001, num_epochs=50000, batch_size=4096, leaky_relu=False):
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 100 == 0:
            print(f"Epoch [{epoch+1}/{num_epochs}], Avg Loss: {avg_loss.item():.6f}, Max: {max_loss.item():.6f}, LR: {optimizer.param_groups[0]['lr']:.6f}")
...
def load_torch_model(file_name="data/simple_nn.pth", input_size=3, hidden_sizes=[128, 128, 128], output_size=3, device=None):
    # In multiprocessing environments with CUDA, force CPU usage for safety
    # unless explicitly specified otherwise
    if device is None:
        # Check if we're in a multiprocessing context
        import os

        if "CUDA_VISIBLE_DEVICES" not in os.environ and hasattr(torch.multiprocessing, "current_process"):
            # Use CPU in worker processes to avoid CUDA issues
            device = torch.device("cpu")
        else:
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    model = SimpleNN(input_size=input_size, hidden_sizes=hidden_sizes, output_size=output_size, device=device)
    model.load_state_dict(torch.load(file_name, map_location=device, weights_only=False))

    # Ensure the model is in float32 for consistency
    model = model.float()

    print(f"PyTorch model {file_name} loaded on device {device}")
    return model
...
def generate_data(input_size, input_domain, delta=0.01, batch_size=256, dynamics_model=None, device=None):
    """
    Generate random data points for training or verification.
    ...
    Args:
        ...
        delta: Unused. (Retained for compatibility.)
    ...
    Returns:
        ...
        y_train: Output data with shape [input_dim, batch_size], or None if no dynamics_model is provided.
    """
