...
def main(system_type="barr1", train=True, verify=False, alpha=1.0, region_type="simplicial", executor_type="single", max_depth=None, activation=None, hidden_sizes=None, save_path=None):
    ...
    if verify:
        ...
        # Convention: data/models/{activation.lower()}_models/{system}_cbf.onnx
        valid_verify_activations = ["Tanh", "Relu", "Sigmoid", "LeakyRelu"]
        if activation not in valid_verify_activations:
            raise ValueError(f"Unsupported activation function for verification: {activation}")
        network_path = f"data/models/{activation.lower()}_models/{dynamics_model.system_name}_cbf.onnx"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save-path", type=str, default="data/models/tanh_models",
                       help="Directory to save trained model (default: data/models/tanh_models)")
