"""
Neural CBF 迭代修复 v10_ibp (LBP验证 + Bounds损失版)

与 main_clean_v9_ibp.py 的区别:
    - 验证部分使用 LBP (verify_cbf) 而非 IBP (IBPVerifier)
    - 计算损失部分保持 main_clean_v9_ibp.py 的写法（使用上下界）

其他内容与 main_clean_v9_ibp.py 一致。
"""
...
def main():
    parser = argparse.ArgumentParser(description='Neural CBF 迭代修复 v10_ibp (LBP验证 + Bounds损失版)')
    ...
    print("=" * 70)
    print(f"Neural CBF 迭代修复 v10_ibp (LBP验证 + Bounds损失版)")
    print(f"  激活={activation}, 系统={system_name_key}")
    print(f"  注意: 此版本使用 LBP 验证，Bounds 计算损失")
    print("=" * 70)
    ...
    if original_max_depth_standard >= 99.9 and original_max_depth_harmonic >= 99.9:
        ...
        import json
        results_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "nr_results_v10_ibp")
        os.makedirs(results_dir, exist_ok=True)
        run_result = {
            'system': system_name_key, 'activation': activation,
            'method': 'Harmonic Mean CBF Pass Rate (v10_ibp, LBP验证 + Bounds损失)',
        ...
        result_file = os.path.join(results_dir, f"result_{system_name_key}_{activation}_v10_ibp.json")
    ...
    pytorch_save_path = f"New_repair/regions/{dynamics_model.system_name}_{activation}_cbf_repaired_v10_ibp.pth"
    torch.save(model.state_dict(), pytorch_save_path)
    onnx_path = f"New_repair/regions/{dynamics_model.system_name}_{activation}_cbf_repaired_v10_ibp.onnx"
    ...
    print(f"\n开始渐进式深度分层修复 (v10_ibp, LBP验证 + Bounds损失), Phase {current_phase}, max_depth={current_max_depth}")
    ...
    for iteration in range(max_total_iterations):
        ...
        if len(failed_safe_simplices) == 0 and len(failed_unsafe_simplices) == 0:
            ...
            for inner_step in range(num_inner_steps):
                ...
                # 使用上下界计算损失 (与 main_clean_v9_ibp.py 一致)
        ...
        # 使用 LBP 验证 (与 main_clean_v8.py 一致)
        ...
        verified_regions_path = f"New_repair/regions/verified_regions_{dynamics_model.system_name}_{activation}_repaired_v10_ibp.pt"
        regions_to_save = {
    ...
    print(f"\n{'='*70}")
    print("修复前后对比 (v10_ibp, LBP验证 + Bounds损失)")
    print(f"{'='*70}")
    print(f"指标                     原始          最终          变化")
    print(f"───────────────────────────────────────────────────────")
