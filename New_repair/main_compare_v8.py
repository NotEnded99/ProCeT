"""
Neural CBF 迭代修复对比实验版: LAST LAYER REPAIR

基于论文 "LAST LAYER REPAIR: A METAMATERIAL-AGNOSTIC METHOD FOR VERIFYABLE..."
的核心思想: 只修复神经网络最后一层的参数，前面的层全部冻结。

与 main_clean_v8.py 的区别:
    - 冻结网络前面所有层，只训练最后一层
    - 优化目标: min ||W - W_0||^2 subject to CBF 约束
    - 保持与 main_clean_v8.py 相同的深度递增修复流程

修复流程 (与 main_clean_v8.py 一致):
...
from New_repair.optimizer_module_v3 import compute_repair_loss_and_grad
...
def freeze_except_last_layer(model):
    ...
    for name, param in model.named_parameters():
        # 最后一层是 model.network[-1]
        # name 格式如: "network.4.weight" 表示最后一层
        if "network" in name:
...
class LastLayerRepairLoss(nn.Module):
    """
    LAST LAYER REPAIR 的核心损失函数。

    论文方法 (Eq. 11-12):
        L_total = α * ||W - W_0||^2 + β * L_CBF

    其中:
        - ||W - W_0||^2: 最小二乘正则项，保持与原始权重的接近度
        - L_CBF: CBF约束违反的惩罚项 (softplus)
    """

    def __init__(self, W_orig, b_orig, alpha_reg=1.0, beta_cbf=1.0):
        super().__init__()
        self.W_orig = W_orig.clone().detach()
        self.b_orig = b_orig.clone().detach() if b_orig is not None else None
        self.alpha_reg = alpha_reg  # 正则化系数
        self.beta_cbf = beta_cbf    # CBF损失系数

    def forward(self, model, cbf_loss_value):
        """
        计算总损失 = α * ||W - W_0||^2 + β * L_CBF
        """
        W_current = model.network[-1].weight
        b_current = model.network[-1].bias

        # 最小二乘正则项: ||W - W_0||^2
        reg_loss = torch.sum((W_current - self.W_orig) ** 2)
        if b_current is not None and self.b_orig is not None:
            reg_loss = reg_loss + torch.sum((b_current - self.b_orig) ** 2)

        # 总损失
        total_loss = self.alpha_reg * reg_loss + self.beta_cbf * cbf_loss_value
        return total_loss, reg_loss.item(), cbf_loss_value
...
def main():
    parser = argparse.ArgumentParser(description='Neural CBF 迭代修复对比实验版: LAST LAYER REPAIR')
    ...
    print("=" * 70)
    print(f"Neural CBF 迭代修复对比实验版: LAST LAYER REPAIR")
    ...
    if original_max_depth_standard >= 99.9 and original_max_depth_harmonic >= 99.9:
        ...
        import json
        results_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "nr_results_compare_v8")
        os.makedirs(results_dir, exist_ok=True)
        run_result = {
            'system': system_name_key, 'activation': activation,
            'method': 'LAST LAYER REPAIR (对比实验版)',
        ...
        result_file = os.path.join(results_dir, f"result_{system_name_key}_{activation}_compare_v8.json")
    ...
    pytorch_save_path = f"New_repair/regions/{dynamics_model.system_name}_{activation}_cbf_repaired_compare_v8.pth"
    torch.save(model.state_dict(), pytorch_save_path)
    onnx_path = f"New_repair/regions/{dynamics_model.system_name}_{activation}_cbf_repaired_compare_v8.onnx"
    ...
    for iteration in range(max_total_iterations):
        ...
        if len(failed_safe_simplices) == 0 and len(failed_unsafe_simplices) == 0:
            ...
            for inner_step in range(num_inner_steps):
                ...
                for unsafe_start in range(0, n_unsafe_fp, 1024):
                    ...
                    chunk_loss, chunk_g_w, chunk_g_b = compute_repair_loss_and_grad_last_layer(
        ...
        verified_regions_path = f"New_repair/regions/verified_regions_{dynamics_model.system_name}_{activation}_repaired_compare_v8.pt"
        regions_to_save = {
    ...
    print(f"\n{'='*70}")
    print("修复前后对比 (LAST LAYER REPAIR 对比实验版)")
    print(f"{'='*70}")
    print(f"指标                     原始          最终          变化")
    print(f"───────────────────────────────────────────────────────")
