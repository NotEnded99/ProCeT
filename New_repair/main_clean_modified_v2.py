"""
main_clean_modified_v2.py: Neural CBF 迭代修复 v2 (增强版)

核心改进: 使用完整的 5 种损失函数（与 train_cbf.py 一致）
...
与 main_clean_v2.py 的区别:
- 使用 compute_combined_loss_and_grad：结合验证区域最坏点 loss + 训练过程动态采样 loss
...
def main():
    # ========== 0. 解析命令行参数 ==========
    parser = argparse.ArgumentParser(description='Neural CBF 迭代修复 v2 (增强版)')
    ...
    parser.add_argument('--loss_type', type=str, default='modified',
    ...
    print("=" * 70)
    print(f"Neural CBF 迭代修复 v2 (增强版) [激活={activation}, 系统={system_name_key}]")
    ...
    for iteration in range(num_iterations):
        ...
        print(f"\n[迭代 {iteration+1}.3] 内循环修复 ({num_inner_steps} 步，{loss_mode_str})...")
        ...
        # ---------- 5.4 保存模型 ----------
        pytorch_save_path = f"New_repair/regions/{dynamics_model.system_name}_{activation}_cbf_clean_modified_v2.pth"
        torch.save(model.state_dict(), pytorch_save_path)
        print(f"\n[迭代 {iteration+1}.4] 保存 PyTorch 模型: {pytorch_save_path}")

        onnx_path = f"New_repair/regions/{dynamics_model.system_name}_{activation}_cbf_clean_modified_v2.onnx"
        ...
        # ---------- 5.6 保存验证结果 ----------
        verified_regions_path = f"New_repair/regions/verified_regions_{dynamics_model.system_name}_{activation}_clean_modified_v2.pt"
    ...
    # ========== 6. 最终总结 ==========
    print(f"\n{'='*70}")
    print("迭代修复完成 - 总结 (clean_modified_v2)")
    print(f"{'='*70}")
    ...
    # ========== 7. 保存运行结果 ==========
    import json
    results_dir = "New_repair/nr_results_clean_modified_v2"
    os.makedirs(results_dir, exist_ok=True)

    # 转换 iteration_results 中的 tensor 为 float
    ...
    run_result = {
        ...
        'note': 'clean_modified_v2: 完整 5 种 loss，直接梯度更新',
    }

    result_file = os.path.join(results_dir, f"result_{system_name_key}_{activation}_clean_modified_v2.json")
...
def _save_artifacts(model, dynamics_model, iteration, certified_percentage, stats,
    ...
    """保存模型和验证结果（提前终止时使用）"""
    pytorch_save_path = f"New_repair/regions/{dynamics_model.system_name}_{dynamics_model.activation_fnc}_cbf_clean_modified_v2.pth"
    torch.save(model.state_dict(), pytorch_save_path)

    onnx_path = f"New_repair/regions/{dynamics_model.system_name}_{dynamics_model.activation_fnc}_cbf_clean_modified_v2.onnx"
    pytorch_to_onnx(model, onnx_path, input_dim=dynamics_model.input_dim)

    verified_regions_path = f"New_repair/regions/verified_regions_{dynamics_model.system_name}_{dynamics_model.activation_fnc}_clean_modified_v2.pt"
    regions_to_save = {
        'V_safe': V_safe,
        'V_unsafe': V_unsafe,
