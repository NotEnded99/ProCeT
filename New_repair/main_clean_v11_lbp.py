"""
Neural CBF 迭代修复 v11_lbp (完整LBP验证 + 完整LBP损失计算，使用McCormick)

与 main_clean_v10_lbp.py 的区别:
    - 验证部分使用完整 LBP (verify_cbf from lbp_neural_cbf.cbf.verify_cbf) 带 McCormick
    - 计算损失部分使用完整 LBP with McCormick
      (使用 McCormick envelope 进行区间乘积计算)

其他内容与 main_clean_v10_lbp.py 一致。
"""
...
# v11_lbp: 使用完整 LBP 验证 (with McCormick) + 完整 LBP 损失计算 (with McCormick)
...
def compute_min_L_with_mccormick(batch, dynamics_model, network_linearizer, device, dtype, h_lb_lbp=None):
    ...
    Args:
        batch: List of region samples (numpy arrays or SimplicialRegion objects)
        ...
        h_lb_lbp: Optional LBP h_lb tensor [batch] for alpha term

    Returns:
        min_L: CBF 条件下界 tensor [batch]
    """
    ...
    # Convert numpy arrays to SimplicialRegion objects if needed
    regions = []
    for verts in batch:
    ...
    # 0. Compute network bounds and partial derivative bounds
    ...
    # 1. Get dynamics affine bounds using Taylor linearization
    ...
    # 2. Get Jacobian J(x) affine bounds from linear bound propagation
    A_L, b_L, A_U, b_U = network_linearizer.get_partial_derivative_bounds()
    J_affine_L, J_affine_U = (A_L, b_L), (A_U, b_U)

    # 3. Compute Lower Bound for Drift Term: J(x)f(x) using McCormick
    eta_drift = 0.5
    ...
    # 4. Compute Lower Bound for Class-K Term
    ...
    # 5. Compute Lower Bound for Control Term: sup_u J(x)g(x)u
    if m > 0:
    ...
    # 6. Find minimum over the region
...
def compute_h_ub_with_mccormick(batch, dynamics_model, network_linearizer, device, dtype):
    ...
    # Compute drift term upper bound using McCormick
    eta_drift = 0.5
...
# =============================================================================
# LBP Loss Computation Functions (with McCormick)
# =============================================================================
...
def compute_repair_loss_and_grad_unsafe_lbp_with_mccormick(
    ...
    for batch_start in range(0, len(regions), BATCH_SIZE):
        ...
        del h_ub_batch, loss_batch, grad_batch
        torch.cuda.empty_cache()
...
def compute_repair_loss_and_grad_safe_lbp_with_mccormick(
    ...
    for batch_start in range(0, len(regions), BATCH_SIZE):
        ...
        del min_L_batch, loss_batch, grad_batch
        torch.cuda.empty_cache()
...
def compute_repair_loss_and_grad_lbp(
    ...
    """
    综合修复损失计算 (完整 LBP with McCormick):
    ...
    if verbose:
        print(f"  [修复损失 LBP with McCormick] total_loss={total_loss:.6f}, |g|={grad_norm:.4f}, "
...
def main():
    parser = argparse.ArgumentParser(description='Neural CBF 迭代修复 v11_lbp (完整LBP验证 + 完整LBP损失，使用McCormick)')
    ...
    print("=" * 70)
    print(f"Neural CBF 迭代修复 v11_lbp (完整LBP验证 + 完整LBP损失，使用McCormick)")
    print(f"  激活={activation}, 系统={system_name_key}")
    print(f"  注意: 此版本使用完整 LBP 验证 + 完整 LBP 损失 (with McCormick)")
    print("=" * 70)
    ...
    print(f"\n[3.1] 原始区域 v11_lbp 指标: HarmonicMeanPassRate={original_max_depth_harmonic:.2f}%, R_safe={original_max_depth_R_safe:.2f}%, R_unsafe={original_max_depth_R_unsafe:.2f}%")
    ...
    if original_max_depth_standard >= 99.9 and original_max_depth_harmonic >= 99.9:
        ...
        import json
        results_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "nr_results_v11_lbp")
        os.makedirs(results_dir, exist_ok=True)
        run_result = {
            'system': system_name_key, 'activation': activation,
            'method': 'Harmonic Mean CBF Pass Rate (v11_lbp, 完整LBP验证 + 完整LBP损失，使用McCormick)',
        ...
        result_file = os.path.join(results_dir, f"result_{system_name_key}_{activation}_v11_lbp.json")
    ...
    pytorch_save_path = f"New_repair/regions/{dynamics_model.system_name}_{activation}_cbf_repaired_v11_lbp.pth"
    torch.save(model.state_dict(), pytorch_save_path)
    onnx_path = f"New_repair/regions/{dynamics_model.system_name}_{activation}_cbf_repaired_v11_lbp.onnx"
    ...
    print(f"\n开始渐进式深度分层修复 (v11_lbp, 完整LBP验证 + 完整LBP损失，使用McCormick), Phase {current_phase}, max_depth={current_max_depth}")
    ...
    for iteration in range(max_total_iterations):
        ...
        verified_regions_path = f"New_repair/regions/verified_regions_{dynamics_model.system_name}_{activation}_repaired_v11_lbp.pt"
        regions_to_save = {
    ...
    print(f"\n{'='*70}")
    print("修复前后对比 (v11_lbp, 完整LBP验证 + 完整LBP损失，使用McCormick)")
    print(f"{'='*70}")
    print(f"指标                     原始          最终          变化")
    print(f"───────────────────────────────────────────────────────")
