"""
Neural CBF 迭代修复 v8_ibp (对比实验版): 简化梯度更新 + IBP min_L 计算

与 main_clean_v8.py 的区别:
    - 使用 IBP (Interval Bound Propagation) 计算 min_L
    - 验证部分使用 IBP (IBPVerifier) 而非 LBP (verify_cbf)
    - 修复目标选择部分使用 compute_simplex_bound_batch_ibp 替代 LBP 版本

其他内容与 main_clean_v8.py 完全一致。
"""
...
from verify_cbf_ibp import compute_simplex_bound_batch_ibp, IBPVerifier
...
def main():
    parser = argparse.ArgumentParser(description='Neural CBF 迭代修复 v8_ibp (IBP min_L版: 简化梯度更新)')
    ...
    print("=" * 70)
    print(f"Neural CBF 迭代修复 v8_ibp (IBP min_L版: 简化梯度更新)")
    print(f"  激活={activation}, 系统={system_name_key}")
    print(f"  注意: 此版本使用 IBP 计算 min_L (select_top_n_v_safe_ibp)")
    print("=" * 70)
    ...
    print(f"\n[3.1] 原始区域 v8_ibp 指标: HarmonicMeanPassRate={original_max_depth_harmonic:.2f}%, R_safe={original_max_depth_R_safe:.2f}%, R_unsafe={original_max_depth_R_unsafe:.2f}%")
    ...
    if original_max_depth_standard >= 99.9 and original_max_depth_harmonic >= 99.9:
        ...
        import json
        results_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "nr_results_v8_ibp")
        os.makedirs(results_dir, exist_ok=True)
        run_result = {
            'system': system_name_key, 'activation': activation,
            'method': 'Harmonic Mean CBF Pass Rate (v8_ibp, IBP min_L)',
        ...
        result_file = os.path.join(results_dir, f"result_{system_name_key}_{activation}_v8_ibp.json")
    ...
    pytorch_save_path = f"New_repair/regions/{dynamics_model.system_name}_{activation}_cbf_repaired_v8_ibp.pth"
    ...
    print(f"\n开始渐进式深度分层修复 (v8_ibp, IBP min_L版), Phase {current_phase}, max_depth={current_max_depth}")
    ...
    for iteration in range(max_total_iterations):
        ...
        if len(failed_safe_simplices) == 0 and len(failed_unsafe_simplices) == 0:
            ...
            for inner_step in range(num_inner_steps):
                ...
                update_norm = simple_gradient_update(model, g_F, current_lr)
        ...
        verified_regions_path = f"New_repair/regions/verified_regions_{dynamics_model.system_name}_{activation}_repaired_v8_ibp.pt"
        regions_to_save = {
    ...
    print(f"\n{'='*70}")
    print("修复前后对比 (v8_ibp, IBP min_L版)")
    print(f"{'='*70}")
    print(f"指标                     原始          最终          变化")
    print(f"───────────────────────────────────────────────────────")
