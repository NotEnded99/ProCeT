"""
Neural CBF 迭代修复 v9_ibp (Bounds版): 使用上下界计算损失替代采样

与 main_clean_v8_ibp.py 的区别:
    - 不再使用采样方法计算损失（extract_feature_points_from_regions）
    - 对 Unsafe 区域使用 LBP 上界 (h_ub) 计算损失
    - 对 Safe 区域使用 IBP 下界 (min_L) 计算损失
    - 使用 optimizer_module_v8 中的新损失函数

其他内容与 main_clean_v8_ibp.py 一致。
"""
...
# v9: 使用上下界计算损失
from verify_cbf_ibp import compute_simplex_bound_batch_ibp, IBPVerifier, IBPNetworkBoundCalculator
from New_repair.optimizer_module_v8 import compute_repair_loss_and_grad_bounds
...
def select_top_n_v_safe_ibp(model, V_safe, dynamics_model, translator, top_n, cbf_margin=0.0):
    """
    使用 IBP 计算 min_L 来选择 top_n 个安全区域

    与 select_top_n_v_safe 的区别：使用 compute_simplex_bound_batch_ibp 替代 LBP 版本
    """
    if len(V_safe) == 0:
        return []
    ...
    for batch_start in range(0, n_available, BATCH_SIZE):
        ...
        # 使用 IBP 版本的 min_L 计算
        min_L_batch = compute_simplex_bound_batch_ibp(
            model, V_safe_batch, 'safe', dynamics_model=dynamics_model, translator=translator
        )
...
def main():
    parser = argparse.ArgumentParser(description='Neural CBF 迭代修复 v9_ibp (Bounds版: 使用上下界计算损失)')
    ...
    print("=" * 70)
    print(f"Neural CBF 迭代修复 v9_ibp (Bounds版: 使用上下界计算损失)")
    print(f"  激活={activation}, 系统={system_name_key}")
    print(f"  注意: 此版本使用 LBP 上界处理 Unsafe，IBP 下界处理 Safe")
    print("=" * 70)
    ...
    print(f"\n[3.1] 原始区域 v9_ibp 指标: HarmonicMeanPassRate={original_max_depth_harmonic:.2f}%, R_safe={original_max_depth_R_safe:.2f}%, R_unsafe={original_max_depth_R_unsafe:.2f}%")
    ...
    if original_max_depth_standard >= 99.9 and original_max_depth_harmonic >= 99.9:
        ...
        import json
        results_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "nr_results_v9_ibp")
        os.makedirs(results_dir, exist_ok=True)
        run_result = {
            'system': system_name_key, 'activation': activation,
            'method': 'Harmonic Mean CBF Pass Rate (v9_ibp, Bounds)',
        ...
        result_file = os.path.join(results_dir, f"result_{system_name_key}_{activation}_v9_ibp.json")
    ...
    pytorch_save_path = f"New_repair/regions/{dynamics_model.system_name}_{activation}_cbf_repaired_v9_ibp.pth"
    torch.save(model.state_dict(), pytorch_save_path)
    onnx_path = f"New_repair/regions/{dynamics_model.system_name}_{activation}_cbf_repaired_v9_ibp.onnx"
    ...
    print(f"\n开始渐进式深度分层修复 (v9_ibp, Bounds版), Phase {current_phase}, max_depth={current_max_depth}")
    ...
    for iteration in range(max_total_iterations):
        ...
        if len(failed_safe_simplices) == 0 and len(failed_unsafe_simplices) == 0:
            ...
            # 使用 Bounds 版本选择修复目标 (仍然使用 IBP 选择)
            top_n_v_safe = select_top_n_v_safe_ibp(model, list(V_safe_init), dynamics_model, translator, top_n_protect)
            ...
            ibp_calculator = IBPNetworkBoundCalculator(model, dtype=torch.float32, device=device)
            ...
            for inner_step in range(num_inner_steps):
                ...
                # 使用上下界计算损失
                # - failed_safe_simplices 使用 IBP 下界 (min_L)
                # - failed_unsafe_simplices 使用 LBP 上界 (h_ub)
                loss_val, g_F = compute_repair_loss_and_grad_bounds(
                    model=model,
                    dynamics_model=dynamics_model,
                    safe_simplices=failed_safe_simplices,
                    unsafe_simplices=failed_unsafe_simplices,
                    lbp_linearizer=lbp_linearizer,
                    ibp_calculator=ibp_calculator,
                    margin=0.1,
                    cbf_margin=0.0,
                    beta=5.0,
                    grad_clip_norm=10.0,
        ...
        pytorch_save_path = f"New_repair/regions/{dynamics_model.system_name}_{activation}_cbf_repaired_v9_ibp.pth"
        torch.save(model.state_dict(), pytorch_save_path)
        print(f"  模型已保存: {pytorch_save_path}")
        onnx_path = f"New_repair/regions/{dynamics_model.system_name}_{activation}_cbf_repaired_v9_ibp.onnx"
        pytorch_to_onnx(model, onnx_path, input_dim=dynamics_model.input_dim)
        ...
        verified_regions_path = f"New_repair/regions/verified_regions_{dynamics_model.system_name}_{activation}_repaired_v9_ibp.pt"
        regions_to_save = {
