"""
main_modified_v2.py: Neural CBF 迭代修复 v2 (增强版 + Jacobian/QP 投影)

核心改进:
1. 使用完整的 5 种损失函数（与 train_cbf.py 一致）
2. 使用 Jacobian 矩阵计算已验证区域的梯度约束
3. 使用 QP 投影保护已验证区域，确保更新不破坏已验证通过的部分

与 main_clean_modified_v2.py 的区别:
- 增加 compute_jacobian_at_worst_points_simple 计算雅可比矩阵
- 使用 qp_project_and_update 进行梯度投影
- 不使用直接梯度下降
...
from New_repair.optimizer_module_clean_v2 import (
    compute_combined_loss_and_grad,
    compute_repair_loss_and_grad_modified,
)
...
def main():
    # ========== 0. 解析命令行参数 ==========
    parser = argparse.ArgumentParser(description='Neural CBF 迭代修复 v2 (增强版 + Jacobian/QP)')
    ...
    parser.add_argument('--lambda_safe', type=float, default=1.0,
                        help='L_safe 权重（默认 1.0）')
    parser.add_argument('--lambda_unsafe', type=float, default=10.0,
                        help='L_unsafe 权重（默认 10.0）')
    parser.add_argument('--lambda_unsafe_max', type=float, default=5.0,
                        help='L_unsafe_max 权重（默认 5.0）')
    parser.add_argument('--lambda_cbf', type=float, default=5.0,
                        help='L_cbf 权重（默认 5.0）')
    parser.add_argument('--safe_margin', type=float, default=0.01,
                        help='安全 margin（默认 0.01）')
    parser.add_argument('--unsafe_margin', type=float, default=0.01,
                        help='不安全 margin（默认 0.01）')
    parser.add_argument('--cbf_margin', type=float, default=0.0,
                        help='CBF margin（默认 0.0）')
    parser.add_argument('--loss_type', type=str, default='combined',
                        choices=['combined', 'modified'],
                        help='损失函数类型: combined=组合损失, modified=验证区域最坏点损失（默认 combined）')
    ...
    lambda_unsafe_max = args.lambda_unsafe_max
    lambda_cbf = args.lambda_cbf
    safe_margin = args.safe_margin
    unsafe_margin = args.unsafe_margin
    cbf_margin = args.cbf_margin
    ...
    print("=" * 70)
    print(f"Neural CBF 迭代修复 v2 (增强版+QP) [激活={activation}, 系统={system_name_key}]")
    print(f"  采样数/单纯形: {num_samples}, 内迭代: {num_inner_steps}, lr: {lr}")
    print(f"  Loss 权重: safe={lambda_safe}, unsafe={lambda_unsafe}, "
          f"unsafe_max={lambda_unsafe_max}, cbf={lambda_cbf}")
    print(f"  Margins: safe={safe_margin}, unsafe={unsafe_margin}, cbf={cbf_margin}")
    print(f"  Loss 类型: {loss_type} (combined=组合损失, modified=验证最坏点)")
    print(f"  使用 Jacobian + QP 投影保护已验证区域")
    print("=" * 70)
    ...
    for iteration in range(num_iterations):
        ...
        # V_unsafe -> unsafe_jac_points（用于 J_unsafe 计算）
        ...
        # J_unsafe: ∂h/∂θ，约束取反（J @ d >= 0 意味着 h 减小）
        ...
        # ---------- 5.3 生成训练样本 ----------
        print(f"\n[迭代 {iteration+1}.3] 生成训练样本...")

        train_safe_points = dynamics_model.safe_set.sample_points(
            min(500, max(10, len(V_safe_init) * 10)), device=device, use_torch=True
        )
        print(f"    训练安全集采样: {train_safe_points.shape[0]} 个点")

        train_unsafe_points = dynamics_model.unsafe_set_interior.sample_points(
            min(500, max(10, len(V_unsafe_init) * 10)), device=device, use_torch=True
        )
        print(f"    训练不安全集采样: {train_unsafe_points.shape[0]} 个点")

        # ---------- 5.4 内循环修复（增强版损失 + QP 投影） ----------
        loss_mode_str = "组合损失" if loss_type == "combined" else "验证最坏点损失"
        print(f"\n[迭代 {iteration+1}.4] 内循环修复 ({num_inner_steps} 步，{loss_mode_str}+QP投影)...")
        ...
        for inner_step in range(num_inner_steps):
            ...
            if loss_type == "combined":
                # 组合损失
                loss_val, g_F, loss_dict = compute_combined_loss_and_grad(
                    model=model,
                    dynamics_model=dynamics_model,
                    safe_worst_points=safe_worst_points,
                    unsafe_worst_points=unsafe_worst_points,
                    cbf_worst_points=cbf_worst_points,
                    train_safe_points=train_safe_points,
                    train_unsafe_points=train_unsafe_points,
                    train_boundary_points=None,
                    translator=translator,
                    lambda_safe=lambda_safe,
                    lambda_unsafe=lambda_unsafe,
                    lambda_unsafe_max=lambda_unsafe_max,
                    lambda_cbf=lambda_cbf,
                    lambda_train_safe=lambda_safe,
                    lambda_train_unsafe=lambda_unsafe,
                    lambda_train_unsafe_max=lambda_unsafe_max,
                    lambda_train_cbf=lambda_cbf,
                ...
                # 验证区域最坏点损失
                loss_val, g_F, loss_dict = compute_repair_loss_and_grad_modified(
                    model=model,
                    dynamics_model=dynamics_model,
                    safe_worst_points=safe_worst_points,
                    unsafe_worst_points=unsafe_worst_points,
            ...
            if inner_step == num_inner_steps - 1 or (inner_step + 1) % 5 == 0:
                loss_info = (f"total={loss_dict.get('total_loss', 0):.6f}, "
                             f"safe={loss_dict.get('safe_loss', 0):.6f}, "
                             f"unsafe={loss_dict.get('unsafe_loss', 0):.6f}, "
                             f"unsafe_max={loss_dict.get('unsafe_max_loss', 0):.6f}, "
                             f"cbf={loss_dict.get('cbf_loss', 0):.6f}")
                if loss_type == "combined":
                    loss_info += (f", train_safe={loss_dict.get('train_safe_loss', 0):.6f}, "
                                  f"train_unsafe={loss_dict.get('train_unsafe_loss', 0):.6f}, "
                                  f"train_cbf={loss_dict.get('train_cbf_loss', 0):.6f}")
                print(f"    [内步 {inner_step+1}/{num_inner_steps}] {loss_info}")
                print(f"           |g|={g_raw_norm:.4f}, |d|={update_norm:.6f}, active={active}")

        # ---------- 5.5 保存模型 ----------
        pytorch_save_path = f"New_repair/regions/{dynamics_model.system_name}_{activation}_cbf_modified_v2.pth"
        torch.save(model.state_dict(), pytorch_save_path)
        print(f"\n[迭代 {iteration+1}.5] 保存 PyTorch 模型: {pytorch_save_path}")

        onnx_path = f"New_repair/regions/{dynamics_model.system_name}_{activation}_cbf_modified_v2.onnx"
        pytorch_to_onnx(model, onnx_path, input_dim=dynamics_model.input_dim)
        print(f"[迭代 {iteration+1}.6] 转换为 ONNX: {onnx_path}")

        # ---------- 5.6 验证 ----------
        print(f"\n[迭代 {iteration+1}.7] 运行验证...")
        ...
        # ---------- 5.7 保存验证结果 ----------
        verified_regions_path = f"New_repair/regions/verified_regions_{dynamics_model.system_name}_{activation}_modified_v2.pt"
        ...
        print(f"[迭代 {iteration+1}.9] 保存验证区域: {verified_regions_path}")
        ...
        iteration_results.append({
            ...
            'loss_dict': inner_history[-1]['loss_dict'] if inner_history else {},
        })

    # ========== 6. 最终总结 ==========
    print(f"\n{'='*70}")
    print("迭代修复完成 - 总结 (modified_v2)")
    print(f"{'='*70}")
    ...
    # ========== 7. 保存运行结果 ==========
    import json
    results_dir = "New_repair/nr_results_modified_v2"
    os.makedirs(results_dir, exist_ok=True)

    def _clean_for_json(obj):
        """递归转换 tensor/numpy 为 Python float/int"""
    ...
    run_result = {
        ...
        'cbf_margin': cbf_margin,
        'loss_type': loss_type,
        'iteration_results': _clean_for_json(iteration_results),
        'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        'note': 'modified_v2: 完整 5 种 loss + Jacobian + QP 投影',
    }

    result_file = os.path.join(results_dir, f"result_{system_name_key}_{activation}_modified_v2.json")
...
def _save_artifacts(model, dynamics_model, iteration, certified_percentage, stats,
    ...
    """保存模型和验证结果（提前终止时使用）"""
    pytorch_save_path = f"New_repair/regions/{dynamics_model.system_name}_{dynamics_model.activation_fnc}_cbf_modified_v2.pth"
    torch.save(model.state_dict(), pytorch_save_path)

    onnx_path = f"New_repair/regions/{dynamics_model.system_name}_{dynamics_model.activation_fnc}_cbf_modified_v2.onnx"
    pytorch_to_onnx(model, onnx_path, input_dim=dynamics_model.input_dim)

    verified_regions_path = f"New_repair/regions/verified_regions_{dynamics_model.system_name}_{dynamics_model.activation_fnc}_modified_v2.pt"
    regions_to_save = {
        'V_safe': V_safe,
        'V_unsafe': V_unsafe,
