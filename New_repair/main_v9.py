...
def select_repair_targets(
    F_h_positive_in_unsafe, F_safe_cbf_violation, F_depth_limit_reached,
    ...
    if current_phase == 1:
        ...
        return (
            list(F_safe_cbf_violation) + list(F_depth_limit_reached),
            list(F_h_positive_in_unsafe) + list(F_unsafe_cannot_split),
            "Phase2_All"
        )
...
def main():
    ...
    original_safety_metrics = compute_safety_metrics_v8(
        ...
        F_safe_cbf_violation_init, F_depth_limit_reached_init, F_unsafe_cannot_split_init,
    )
