"""
Neural CBF 迭代修复 v8 (对比实验版): 简化梯度更新
...
def main():
    parser = argparse.ArgumentParser(description='Neural CBF 迭代修复 v8 (对比实验版: 简化梯度更新)')
    ...
    print("=" * 70)
    print(f"Neural CBF 迭代修复 v8 (对比实验版: 简化梯度更新)")
    print(f"  激活={activation}, 系统={system_name_key}")
    print(f"  注意: 此版本不使用 Jacobian 和梯度投影，直接梯度更新")
    print("=" * 70)
    ...
    if original_max_depth_standard >= 99.9 and original_max_depth_harmonic >= 99.9:
        ...
        import json
        results_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "nr_results_v8_clean")
        os.makedirs(results_dir, exist_ok=True)
        run_result = {
            'system': system_name_key, 'activation': activation,
            'method': 'Harmonic Mean CBF Pass Rate (v8, 简化梯度更新)',
        ...
        result_file = os.path.join(results_dir, f"result_{system_name_key}_{activation}_v8_clean.json")
    ...
    pytorch_save_path = f"New_repair/regions/{dynamics_model.system_name}_{activation}_cbf_repaired_v8_clean.pth"
    torch.save(model.state_dict(), pytorch_save_path)
    onnx_path = f"New_repair/regions/{dynamics_model.system_name}_{activation}_cbf_repaired_v8_clean.onnx"
    ...
    print(f"\n开始渐进式深度分层修复 (简化梯度更新版), Phase {current_phase}, max_depth={current_max_depth}")
    ...
    for iteration in range(max_total_iterations):
        ...
        verified_regions_path = f"New_repair/regions/verified_regions_{dynamics_model.system_name}_{activation}_repaired_v8_clean.pt"
        regions_to_save = {
