"""
Neural CBF 迭代修复 v5：RS Jacobian + RS 损失梯度

核心改进（相比 v4）：
    - 损失梯度也使用随机平滑（RS）方法，替代 autograd.backward()
    - 与 RS Jacobian 完全对齐：都对 Crown 下界求梯度
    - 两者目标函数一致，不再存在真实函数 vs Crown 下界的不一致问题
...
def main():
    # ========== 0. 解析命令行参数 ==========
    parser = argparse.ArgumentParser(description='Neural CBF 迭代修复 v5 (RS Jacobian + RS 损失梯度)')
    ...
    print("=" * 70)
    print(f"Neural CBF 迭代修复 v5 [RS Jacobian + RS 损失梯度]")
    print(f"  激活={activation}, 系统={system_name_key}")
    print(f"  RS 参数: N={rs_n}, sigma={rs_sigma} (Jacobian 和损失共用)")
    ...
    print(f"\n{'='*70}")
    print(f"开始 {num_iterations} 次迭代修复 (v5: RS Jacobian + RS 损失梯度)")
    print(f"{'='*70}")
    ...
    for iteration in range(num_iterations):
        ...
        if total_fail == 0:
            ...
            pytorch_save_path = f"New_repair/regions/{dynamics_model.system_name}_{activation}_cbf_repaired_v5.pth"
            torch.save(model.state_dict(), pytorch_save_path)
            print(f"\n[迭代 {iteration+1}] 保存 PyTorch 模型: {pytorch_save_path}")

            onnx_path = f"New_repair/regions/{dynamics_model.system_name}_{activation}_cbf_repaired_v5.onnx"
            pytorch_to_onnx(model, onnx_path, input_dim=dynamics_model.input_dim)
            print(f"[迭代 {iteration+1}] 转换为 ONNX: {onnx_path}")

            verified_regions_path = f"New_repair/regions/verified_regions_{dynamics_model.system_name}_{activation}_repaired_v5.pt"
            regions_to_save = {
                'V_safe': V_safe_init,
        ...
        # ========== 5.1 准备 failure simplices 列表（用于 RS 损失梯度）==========
        print(f"\n[迭代 {iteration+1}.1] 准备 failure simplices 列表...")

        # F_h_positive_in_unsafe（障碍区违规）
        failed_unsafe_simplices = list(F_h_positive_in_unsafe_init)
        print(f"    F_h 违规单纯形: {len(failed_unsafe_simplices)} 个")

        # CBF 违规区域（safe 类型的 failure）
        failed_safe_simplices = (
            list(F_safe_cbf_violation_init) +
            list(F_depth_limit_reached_init) +
            list(F_unsafe_cannot_split_init)
        )
        print(f"    CBF 违规单纯形: {len(failed_safe_simplices)} 个 (safe_cbf_violation={len(F_safe_cbf_violation_init)}, "
              f"depth_limit={len(F_depth_limit_reached_init)}, unsafe_cannot_split={len(F_unsafe_cannot_split_init)})")
        ...
        # ========== 5.3 内循环修复（RS 损失梯度 + QP 投影）==========
        print(f"\n[迭代 {iteration+1}.3] 内循环修复 ({num_inner_steps} 步)...")

        inner_history = []
        for inner_step in range(num_inner_steps):
            # 计算修复损失（RS 方法：failure simplices + RS 梯度）
    ...
    # ========== 6. 最终总结 ==========
    print(f"\n{'='*70}")
    print("迭代修复完成 - 总结 (v5: RS Jacobian + RS 损失梯度)")
    print(f"{'='*70}")
    ...
    # ========== 7. 保存结果到 JSON ==========
    import json
    results_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "nr_results_v5")
    os.makedirs(results_dir, exist_ok=True)

    run_result = {
        'system': system_name_key,
        'activation': activation,
        'method': 'RS Jacobian + RS Loss Gradient',
        'rs_n': rs_n,
        'rs_sigma': rs_sigma,
    ...
    result_file = os.path.join(results_dir, f"result_{system_name_key}_{activation}_v5.json")
